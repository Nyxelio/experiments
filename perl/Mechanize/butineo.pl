...
#TEST DU 01/03/2013
#my $cookie_jar = $m->cookies();
#set_cookie ?
#$cookie_jar->set_cookie(0,'VisitId','356541181','/','butineo.fr',80,0,0,86400,0);
...
#print "appel popin\n";
#$m->follow_link( tag => 'a', href => '/logon/popin' ); #<-----NE VA PAS AU DELA
sleep(2);
#$m->follow_link( tag => 'a', href => '#' );
#sleep(2);

#$m->get($link."/logon/popin");
#sleep(2);
#print "ok\n";

#=begin 
print "appel class\n";
#$m->follow_link( tag => 'a', class => 'openFancyboxpopin', synchronize => 0 );
#24/02/2013
$m->follow_link( tag => 'a', class => 'openFancybox', synchronize => 0 );
sleep(2);
print "ok\n";


#$m->follow_link( tag => 'a', class => 'openFancybox');
#$m->follow_link( tag => 'a', href => '/logon/popin', synchronize => 0);
#$m->follow_link( xpath => '//ul[@class="links"]/li/a[@class="openFancybox"]', synchronize => 0);
#sleep(2);
#print "ok\n";
...
print "appel page tout les lots: ";

$m->get($link."/tous-les-lots");
...
do {
    ...
    #recup de tout les liens de la page
    @sublinks = $tree->findnodes_as_strings( '//tr/td[@class="col1"]/div[@class="text"]/strong[@class="title"]/a/@href');

    #si une page suivante
    $nextlink = $tree->findvalue('//a[text()="Next"]/@href');
...
while (length($nextlink) > 0);

print "Nombre de liens a jouer: ".@links."\n\n";

#31/01: classement par date ? recup image ?
#numerotation
...
foreach my $sublink (@links)
{
    ...
    #Si des checkbox a remplir
    my @radios = $tree->findnodes_as_strings( '//label[starts-with(text(),"Non")]/@for');
    ...
    $m->click_button( value => 'Participer au tirage au sort' );
    sleep(2);

    
    #Page retour de résultat
    $tree = HTML::TreeBuilder->new_from_content ( $m->content );

    $value = $tree->findvalue('//span[@class="mark"]/strong/text()');
...
sub Connexion
{
    ...
    #connexion OK
    $m->form_id('FormLogOn');
    $m->field( Email => 'datenshi33@gmail.com' );
    $m->field( Password => 'k3stnWM7' );
    $m->click_button( id => 'loginSubmit' );
    #$m->submit();
