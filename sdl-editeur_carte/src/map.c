     ...
     switch(event->type)
     {
      case SDL_KEYDOWN:
	    if(event->key.state==SDL_PRESSED)
	    {
          ...
          {
               SaveMap(map, "../data/map/save2.map", "../data/musique/Opening1.mid", "../data/tiles/mchip0.bmp");
          }
          else if(event->key.keysym.sym==SDLK_l) /* LOAD */
          {
               f=fopen("../data/map/01.map","r");
               if(f)
               {
                  fclose(f);
                  FreeMap(map);
...
fprintf(stderr,"3\n");
	...
	*/
	fprintf(stderr,"4\n");
     ...
     map->surf_chipset=SDL_LoadBMP("../data/tiles/mchip0.bmp");
     SDL_SetColorKey(map->surf_chipset, SDL_SRCCOLORKEY, SDL_MapRGB(map->surf_chipset->format,8,33,82));
